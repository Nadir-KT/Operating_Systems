...
function ToggleAllElements(ids, linkTextID, imgID, collapseAllText, expandAllText)
{
    ...
    if (img != null) {
        if (currentSectionStateExpanded) {
            setNewImageSource(img, "btn_collapse.gif");
        } else {
            setNewImageSource(img, "btn_expand.gif");
        }
    }
}
...
function onBodyLoadEx(framesetfile, targetframe, topicfile)
{
    ...
    onBodyLoad();
    synchTOC(topicfile);
    if ((document.title) && (parent.frames.length > 0)) {
      if (top.document.title != title_prefix + document.title) {
        top.document.title = title_prefix + document.title;
      }
    }
    return true;
}
