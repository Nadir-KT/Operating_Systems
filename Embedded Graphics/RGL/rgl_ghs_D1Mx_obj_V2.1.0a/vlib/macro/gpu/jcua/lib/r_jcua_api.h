...
/***************************************************************************
  ...
  R_JCUA_ERR_BUS_TIMEOUT              - Time-out error has occurred at bus reset.
...
/***************************************************************************
  ...
  Values:
  R_JCUA_SUB_SAMPLING_1_1             - No sub-sampling.
  R_JCUA_SUB_SAMPLING_1_2             - Sub-samples output data into 1/2.
  R_JCUA_SUB_SAMPLING_1_4             - Sub-samples output data into 1/4.
  R_JCUA_SUB_SAMPLING_1_8             - Sub-samples output data into 1/8.
*/
...
/***************************************************************************
  ...
  R_JCUA_ERR_JDC_IMAGE_SIZE           - Image size error detected. There is a possibility that the
                                        stride value which is set to Stride member of
                                        <r_jcua_FrameBuffer_t> structure is wrong or the size of
                                        frame buffer is smaller than decoding image size. If this
                                        error occurs, sets a right value to Stride member of 
                                        <r_jcua_FrameBuffer_t> structure or re-allocate the frame
                                        buffer, then calls the <R_JCUA_DecoderStart> function.
...
/***************************************************************************
  ...
  Address - The address of frame buffer. This address must be a multiple of 8 bytes.
            Specifying Local RAM area is prohibited.
  Size    - The size of frame buffer. This address must be a multiple of 8 bytes.
            This value must match the complete decoded image size, also if output division.
  Stride  - The stride of frame buffer. This stride must be a multiple of 8 bytes.
            mode is used.If the stride sets to zero, that is reset to number which decoded
...
/***************************************************************************
  ...
  Description:  
  This type describes the decode with sub-sample parameter information.
  
  Struct members:
  VertlSubSampling - Horizontal Sub-sampling. See <r_jcua_SubSampling_t>.
  HorizSubSampling - Vertical Sub-sampling. See <r_jcua_SubSampling_t>.
...
/***************************************************************************
  ...
  <R_JCUA_ERR_NOT_ACCEPTABLE>. If any decoding error (the callback reason is 
  <R_JCUA_CB_FACTOR_DECODE_ERRORED>, <R_JCUA_CB_FACTOR_DECODE_SIZEOVER>, 
  <R_JCUA_CB_FACTOR_HEADER_TIMEOUT>, <R_JCUA_CB_FACTOR_DECODE_TIMEOUT> or 
  <R_JCUA_CB_FACTOR_FATAL_ERROR>) occurred after calling this function, the JCUA driver must 
  re-do from the Initialization (<R_JCUA_Init> function).
  Note that this function can't call from interrupt handler.
...
/***************************************************************************
  ...
  Description:
  This function is executed in response to the time-out interrupt.
