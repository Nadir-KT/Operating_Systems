...
FILE    : $Id: r_dev_pin.c 16858 2019-08-06 07:14:15Z shinya.tomari.wj@renesas.com $
============================================================================
DESCRIPTION
D1Mx pin functions
============================================================================
...
static const r_dev_PortInputCapability_t loc_PortCapabilityD1M2[] = 
...
static const r_dev_PortInputCapability_t loc_PortCapabilityD1M1[] = 
/*  Port    InputType11,        InputType10,        InputType01,        InputType00 */
{
    {0u,    R_DEV_PIN_TTL,      R_DEV_PIN_TTL,      R_DEV_PIN_SCHMITT4, R_DEV_PIN_SCHMITT1},
    {1u,    R_DEV_PIN_CMOS1,    R_DEV_PIN_CMOS1,    R_DEV_PIN_SCHMITT4, R_DEV_PIN_SCHMITT1},
    ...
    {42u,   R_DEV_PIN_CMOS1,    R_DEV_PIN_CMOS1,    R_DEV_PIN_SCHMITT4, R_DEV_PIN_SCHMITT1},
    {43u,   R_DEV_PIN_CMOS1,    R_DEV_PIN_CMOS1,    R_DEV_PIN_SCHMITT4, R_DEV_PIN_SCHMITT1},
    {44u,   R_DEV_PIN_CMOS1,    R_DEV_PIN_CMOS1,    R_DEV_PIN_SCHMITT4, R_DEV_PIN_SCHMITT1},
    {45u,   R_DEV_PIN_CMOS1,    R_DEV_PIN_CMOS1,    R_DEV_PIN_SCHMITT4, R_DEV_PIN_SCHMITT1},
...
uint8_t R_DEV_PinConfig(const r_dev_PinConfig_t * PinCfg)
{
    ...
    const r_dev_PortInputCapability_t   *capability;
    ...
    if ((dev >= R_DEV_R7F701408) && (dev <= R_DEV_R7F701412))
    {
        capability = loc_PortCapabilityD1M2;
    }
    else
    {
        capability = loc_PortCapabilityD1M1;
    }
    
    do {
        if (capability[port_position].Port == PinCfg->Port)
        {
            if (capability[port_position].InputType00 == PinCfg->InputType)
            {
                PisaRegBitValue = 0;
                PisRegBitValue  = 0;
            }
            if (capability[port_position].InputType01 == PinCfg->InputType)
            {
                PisaRegBitValue = 0;
                PisRegBitValue  = 1;
                if (capability[port_position].InputType01
                    == capability[port_position].InputType00)
                {
                    portHasPisRegister  = 0;
                }
            }
            if (capability[port_position].InputType10 == PinCfg->InputType)
            {
                PisaRegBitValue = 1;
                PisRegBitValue  = 0;
            }
            if (capability[port_position].InputType11 == PinCfg->InputType)
            {
                PisaRegBitValue = 1;
                PisRegBitValue  = 1;
                if (capability[port_position].InputType11
                    == capability[port_position].InputType10)
                {
                    portHasPisRegister  = 0;
                }
            }
    ...
    while (0xFFu != capability[port_position].Port);
    
    if (0 != portHasPisaRegister)
    {
        addr    =   R_DEV_PinGetRegAddr(R_DEV_PIN_REG_PISA, PinCfg->Port);
        reg_val =   R_DEV_READ_REG(16, addr);
