...
FILE    : $Id: r_dev_clk.c 17029 2019-12-24 04:05:18Z shinobu.uehara.xc@renesas.com $
============================================================================
DESCRIPTION
...
*/
static volatile uint32_t LocPbusRatio = 2;
...
/*******************************************************************************
  Function: loc_CalcFreqPlln
    ...
    *FreqOut      - desired output frequency in Mhz
                    Output frequency describes the frequency available to the system.
                    I.e. if a device has a fixed DIV2 after the PLL, provide the fequency you expect after the DIV2.
...
static r_Error_t loc_CalcFreqPll0(uint32_t FrqIn, loc_PllCfgReg_t * PllCfgReg, uint32_t *FrqOut)
{
    ...
    */
    ...
    /* These functions can cover all devices of the d1l/d1m family. */
    /* Some devices have a different HW-configurations for PLL0. These devices run PLL0 with twice the speed, but divide by 2 afterwards. */
    /* For these devices, the register values to achieve 960MHz are the same as for the others to achieve 480MHz. */
    /* As the 'faster PLL' devices have a DIV2, they result in the same frequency output range as all devices. */
    /* Therefore, this function expects, also for the 'faster PLL' devices, to configure it for the frequency you expect to come out after the DIV2. */
    if (freq > 480u)
    {
    ...
    */
    ...
    if ((n_r > 23) && (n_r < 161) )
    {
        *FrqOut       = (FrqIn * n_r)/(m_r * p_r);
        ...
        /* D1M2(H): The PLL1 is followed by a DIVIDE 2 in hardware, so in the end, */
...
r_Error_t R_DEV_PllCfg(uint32_t FrqIn, r_dev_ClkGen_t Pll, const r_dev_PllCfg_t *PllCfg)
{
    ...
    if (R_ERR_OK == ret_val)
    {
        ...
        /* Later done by R_DEV_ClkGenInitStore -> loc_PllCfgStore */
