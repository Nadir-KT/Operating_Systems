...
FILE    : $Id: r_bsp_gfx_mipi_csi2.c 16482 2018-12-07 10:16:09Z matthias.nippert@renesas.com $
============================================================================
DESCRIPTION
        ...
        for (;;) 
{
    ...
    */
    ...
    /* 2. step */
    regval32 = 0x0u;
    regval32 |= (uint32_t)(1ul<<29u);    /* enable clock line noise filter */

    if(2 == NoOfLanes)
    {
        regval32 |= (uint32_t)(1ul<<27u);    /* use 2 data lanes */
    }
    else
    {
        regval32 &= ~(uint32_t)(1ul<<27u);   /* use 1 data lane */
    }
  
    regval32 &= ~(uint32_t)(1ul<<25u);   /* Select FIFO method for PHY layer sync        */
    regval32 |= (uint32_t)(1ul<<24u);    /* Enable reset for clock lane (after time condition)   */
    regval32 |= (uint32_t)(1ul<<23u);    /* Enable reset for data lanes (after time condition)   */
    regval32 &= ~(uint32_t)(1ul<<22u);   /* MIPIADCLK       is T_HS-SETTLE count clock   */
    /* regval32 |= (uint32_t)(1ul<<22u) Ext. clock div4 is T_HS-SETTLE count clock   */
    regval32 |= (uint32_t)(2ul<<16u);    /* RAW8 Input format spec                       */


    /* regval32 |= (1ul<<4u);     disable VTIM             */

    regval32 |= (uint32_t)(1ul<<1u);     /* Enable ECC check and correction              */
    regval32 |= (uint32_t)(1ul<<0);     /* Enable CRC package data check                */
    ...
    regval32 = 0;
    regval32 |= (uint32_t)(1ul<<24u);    /* Enable noise removal mode. */
    regval32 |= (uint32_t)(0x5ul<<17u);  /* count value for noise removal (recommended value is 05H) */
    /* regval32 |= (uint32_t)(1ul<<16u)  LP buffer turns on after LP_EN_ON_WC[15:0] read words */
